...
describe('useBookingCancellation error', () => {
  beforeEach(() => {
  ...
  it('shows a cancellation error', async () => {
    cancelMyBookingMock.mockRejectedValue(new Error('Cancellation failed'))
    ...
    expect(setErrorMessageMock).toHaveBeenLastCalledWith('Cancellation failed')
  ...
  it('does not reload bookings after an error', async () => {
    cancelMyBookingMock.mockRejectedValue(new Error('Cancellation failed'))
  ...
  it('keeps the dialog open after an error', async () => {
    cancelMyBookingMock.mockRejectedValue(new Error('Cancellation failed'))
